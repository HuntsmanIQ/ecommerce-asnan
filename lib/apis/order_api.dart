...
class OrderApi {
  static Future<ResponseModel<OrdersResponse>> getOrders(key, page) async {
    var url =
        "${AppConfig.apiUrl}/customer-order-history?status=$key&page=$page";
    ...
    if (response.result) {
      return ResponseModel(
          response.statusCode, ordersResponseFromJson(response.body));
    } else {
      return ResponseModel(response.statusCode, OrdersResponse.init());
    }
  }

  static Future<ResponseModel<OrderDetailsResponse>> getDetails(
      {required code}) async {
    ...
    if (response.result) {
      return ResponseModel(
          response.statusCode, orderDetailsResponseFromJson(response.body));
    } else {
      return ResponseModel(
          response.statusCode, orderDetailsResponseFromJson(""));
    }
  }

  static Future<ResponseModel<OrderSummeryResponse>> getSummery(
    ...
    var postBody = jsonEncode({"logistic_zone_id": logistic_zone_id});
    ApiResponse response =
        await ApiRequest.post(url: url, header: header, body: postBody);

    if (response.result) {
      return ResponseModel(
          response.statusCode, orderSummeryResponseFromJson(response.body));
    } else {
      return ResponseModel(
          response.statusCode,
  ...
  static Future<ResponseModel<LogisticsResponse>> getLogistics(
      {required cityId}) async {
    var url = "${AppConfig.apiUrl}/logistic-by-city";
    Map<String, String> header = getCommonHeader();
    header.addAll({"Authorization": "Bearer ${access_token.$}"});
    var postBody = jsonEncode({"city_id": cityId});
    ApiResponse response =
        await ApiRequest.post(url: url, header: header, body: postBody);
    print(response.body);
    if (response.result) {
      return ResponseModel(
          response.statusCode, logisticsResponseFromJson(response.body));
    } else {
  ...
  static Future<ResponseModel<OrderCreateResponse>> createOrder(
    ...
    if (response.result) {
      return ResponseModel(
          response.statusCode, orderCreateResponseFromJson(response.body));
    } else {
      return ResponseModel(response.statusCode, OrderCreateResponse.init());
    }
  }

  static Future<ResponseModel<OrderCreateResponse>> createOrderByCOD(
      {required shippingId,
      required billingId,
      required phone,
      required alternativePhone,
  ...
  static Future<ResponseModel<OrderCreateResponse>> createOrderByWallet(
      {required shippingId,
      required billingId,
      required phone,
      required alternativePhone,
  ...
  static Future<ResponseModel<TimeSlotResponse>> getTimeSlot() async {
    ...
    if (response.result) {
      return ResponseModel(
          response.statusCode, timeSlotResponseFromJson(response.body));
    } else {
      return ResponseModel(
          response.statusCode, TimeSlotResponse(days: "0", timeSlots: []));
    }
  }

  static Future<ResponseModel<List<PaymentTypesResponse>>>
    ...
    if (response.result) {
      return ResponseModel(
          response.statusCode, paymentTypesResponseFromJson(response.body));
    } else {
  ...
  static Future<ResponseModel<TrackOrderResponse>> trackOrder(id) async {
    ...
    if (response.result) {
      return ResponseModel(
          response.statusCode, trackOrderResponseFromJson(response.body));
    } else {
